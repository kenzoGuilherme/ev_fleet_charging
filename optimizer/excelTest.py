...
# print(df_Vehicles.loc[0,"departure"])


# Definição do set EVS

df_Vehicles = pd.read_excel("data.xlsx", sheet_name="Vehicles")
evs = set()
for x in  range(0, len(list(df_Vehicles.arrival))):
    print(df_Vehicles.loc[x,"Name"])
    evs.add(df_Vehicles.loc[x,"Name"])

# print(df_Vehicles)
df_Vehicles.set_index("Name", inplace = True)
for ev in evs: 
    print(ev, ".....", df_Vehicles.loc[ev,"SoCini"])
...
for connectors in c:
    print(connectors, ".....", df_Connector.loc[connectors,"Pmax"])
